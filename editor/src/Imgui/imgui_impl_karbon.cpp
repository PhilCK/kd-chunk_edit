...
ImGui_ImplKarbon_NewFrame()
{
        ...
        if(evt & KD_EVENT_INPUT_MS) {
                ...
                auto &io = ImGui::GetIO();
                io.MousePos = ImVec2((float)ms_desc.ms_state[0].x, (float)ms_desc.ms_state[0].y);
                
                bool left_down = !!(ms_desc.ms_state[0].keys[KD_MS_LEFT] & KD_KEY_DOWN);
                io.MouseDown[0] = left_down;
                
                bool right_down = !!(ms_desc.ms_state[0].keys[KD_MS_RIGHT] & KD_KEY_DOWN);
